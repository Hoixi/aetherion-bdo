...
export async function GET(req: Request) {
  return withApp(req, async () => {
    ...
    const secim = { id: true, familyName: true, class: true, avatarUrl: true, guild: { select: { tag: true } } };
    ...
    const uniq = new Map<number, { id: number; familyName: string; class: string; guild: string | null; avatar: string | null }>();
    for (const u of [...rows.map((r) => r.user), ...web]) uniq.set(u.id, { id: u.id, familyName: u.familyName, class: u.class, guild: u.guild?.tag ?? null, avatar: u.avatarUrl || null });
