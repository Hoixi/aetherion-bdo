...
export default function ForumPage() {
  ...
  return (
    <TestShell
      ...
      aside={
        <Link href="/forum/yeni" className="t-tab" data-on>
        ...
        <div className="space-y-4 lg:sticky lg:top-[84px]">
          ...
          {busiest.length > 0 && (
            <Card className="overflow-hidden">
              ...
              {busiest.map((p) => (
                <Link key={p.id} href={`/forum/${p.id}`} className="t-row px-4 py-2.5 block">
...
function PostRow({ p }: { p: Post }) {
  return (
    <Link href={`/forum/${p.id}`} className="t-row px-5 py-4 flex gap-4 items-start">
      {p.author.avatarUrl ? (
