...
import { Swords, Castle, Users, Gauge, ArrowRight, ExternalLink } from "lucide-react";
    ...
    <main className="relative min-h-screen overflow-x-hidden">
      ...
      {duvar && (
        <div className="fixed inset-0 pointer-events-none" aria-hidden
             style={{
               background:
                 "linear-gradient(180deg, rgba(0,0,0,0.62) 0%, rgba(0,0,0,0.78) 55%, var(--t-canvas) 100%)",
             }} />
      )}
      ...
      <div className="relative mx-auto w-full max-w-[880px] px-6 py-14 sm:py-20">
        ...
        <header className="text-center">
          ...
          <p className="mt-3 text-[15px] sm:text-[16px] font-medium" style={{ color: "var(--t-gold)" }}>
            Her gece node war. Her savaş kayıt altında.
          </p>

          <p className="mt-4 mx-auto max-w-[560px] text-[13.5px] leading-relaxed"
             style={{ color: "var(--t-dim)" }}>
            Aetherion bir <b style={{ color: "var(--t-text)" }}>PvP klanıdır</b>. Her gün
            node war atmaya çalışırız; girdiğimiz her savaşın raporunu tutar, herkesin
            katkısını tek tek ölçeriz. Burada kim ne yaptıysa görünür.
          </p>
...
function Kutu({ icon: Icon, etiket, deger, alt }: {
  ...
  return (
    <div className="rounded-[var(--t-r)] px-4 py-3.5"
         style={{ background: "var(--t-surface)", border: "1px solid var(--t-line)" }}>
      <div className="flex items-center gap-1.5">
        <Icon className="w-3.5 h-3.5" strokeWidth={2} style={{ color: "var(--t-gold)" }} />
        <span className="text-[11px]" style={{ color: "var(--t-faint)" }}>{etiket}</span>
      </div>
      <div className="mt-1.5 t-num text-[22px] font-bold leading-none">
        {deger ?? <span className="inline-block w-12 h-[18px] rounded animate-pulse"
                        style={{ background: "var(--t-raised)" }} />}
      </div>
      {alt && <div className="mt-1 text-[10.5px]" style={{ color: "var(--t-faint)" }}>{alt}</div>}
    </div>
  );
}
