...
function Who({ a, size = 32 }: { a: Author; size?: number }) {
  return (
    <Link href={`/test/uyeler/${a.id}`} className="flex items-center gap-2 min-w-0 group">
      {a.avatarUrl ? (
...
export default function ForumPostPage() {
  ...
  return (
    <TestShell
      ...
      aside={
        <Link href="/test/forum" className="t-tab">
                ...
                <div key={c.id} className="px-5 py-4 flex gap-3"
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <Link href={`/test/uyeler/${c.author.id}`}
