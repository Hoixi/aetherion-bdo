/**
 * Eşyanın pazar fiyatı — tek kaynak.
 *
 * Grind tracker sayfası ve masaüstü uygulaması aynı çekiciyi ve aynı
 * önbelleği kullanıyor; eskiden yalnızca API rotasının içindeydi.
 */
/**
...
export type { Kayit as PriceRecord };

/** Önbellekli giriş noktası — rotalar bunu çağırır, fiyatiBul'u değil. */
export async function getMarketPrice(itemId: string): Promise<Kayit> {
...
export async function fiyatiBul(itemId: string): Promise<Kayit> {
  try {
