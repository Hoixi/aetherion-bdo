/**
 * Hasar raporu — ekran görüntüsünü Gemini'ye okutup WarPerformance'a yazar.
 * Site (oturum) ve uygulama (Bearer) uçları aynı çekirdeği kullanır.
 */
...
/** Görüntüyü analiz et, satırları kaydet, partide olup raporda görünmeyenlerin devamsızlığını artır */
export async function islemeRapor(warId: number, base64: string, mimeType: string, shouldClear: boolean) {
  ...
  return { rows: savedRows, total: savedRows.length };
}

/** Rapor satırları + partide seçilip raporda görünmeyenler */
export async function raporuGetir(warId: number) {
